...
\begin{figure}[!htp]
<<chunk3,  results='hide', fig.height=6.5, fig.width=10, tidy=FALSE>>=
...
\begin{figure}[!htp]
<<chunk4,  results='hide', fig.height=6.5, fig.width=10, tidy=FALSE>>=
...
Observe that the x-axes of the variables depth and table in Figure~\ref{fig:tp1} are broken. The reason is that otherwise the lengths of the bars are hard to tell apart. The argument {\tt bias\_brokenX} can be set to determine when a broken x-axis is applied. See Appendix~\ref{secbrokenx} for details.
...
\begin{figure}[htp]
<<chunk7,  results='hide', fig.height=6.5, fig.width=10, tidy=FALSE>>=
...
\begin{figure}[htp]
<<chunk8,  results='hide', fig.height=6.5, fig.width=10, tidy=FALSE>>=
